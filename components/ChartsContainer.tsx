...
export const ChartsContainer: React.FC<ChartsProps> = ({ data }) => {
  ...
  // 2. Prepare Data for "Competency Heatmap Proxy" (Competencies per Country)
  // NOW USES 'tipoHabilidad' directly from the record (which is either from CSV or auto-calculated in parseCSV)
  const countryCompetencyData = useMemo(() => {
    ...
    data.forEach(d => {
      ...
      const type = d.tipoHabilidad; // Use the normalized field
      ...
      if (type === 'Cloud/DevOps') {
        ...
        // Defaults to Tecnico for 'Técnico/Core' or others
  ...
  // 3. Prepare Ranking (Top Colaboradores - Aggregated by Unique Person)
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
      ...
      {/* Chart 2: Ranking Top Talent */}
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        <h3 className="text-lg font-semibold text-slate-800 mb-4">Top Colaboradores (Progreso Promedio)</h3>
        <div className="h-72 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart
              layout="vertical"
      ...
      {/* Chart 3: Competency Distribution by Country (Stacked) */}
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 lg:col-span-2">
        <div className="flex justify-between items-start mb-4">
          <h3 className="text-lg font-semibold text-slate-800">Distribución de Tipos de Habilidad</h3>
          <div className="group relative">
             ...
             <div className="absolute right-0 top-6 w-72 bg-white border border-slate-200 p-3 rounded-lg shadow-xl z-50 text-xs hidden group-hover:block">
                ...
                <p className="text-slate-500 mb-2">Basado en la columna <strong>"Tipo Habilidad"</strong> del CSV.</p>
                <p className="text-slate-400 italic">Si el CSV no tiene esta columna, el sistema la estima automáticamente según palabras clave en la competencia.</p>
             </div>
          </div>
        </div>
        ...
        <div className="h-80 w-full">
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <Bar dataKey="SoftSkills" stackId="a" fill="#f59e0b" name="Soft Skills/Agile" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
